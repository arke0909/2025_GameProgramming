...
bool ResourceManager::Init()
{
	...
	//_resourcePath = curPath.parent_path() / L"Output\\build\\Resource\\";
...
void ResourceManager::Release()
{
    ...
    for (iter = _mapTexture.begin(); iter != _mapTexture.end(); ++iter)
...
{
    wstring fontPath = _resourcePath;
    fontPath += _path;
...
void ResourceManager::LoadTexture(const wstring& _key, const wstring& _path)
{
    ...
    _mapTexture.insert({ _key,pTex }); // 맵에 저장
}

Texture* ResourceManager::GetTexture(const wstring& _key)
{
    auto iter = _mapTexture.find(_key);
